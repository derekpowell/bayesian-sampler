...
dsim <- expand_grid(
  ...
  mutate(
    samps = map2(p, N, ~rBinEssInit(1000, .x, .y, .8)),
    approx = map2(p, N, ~rbeta(1000, 1+ .x*.y, 1+(1-.x)*.y)),
    )

plt_init <- dsim %>% 
  unnest(samps, approx) %>% 
  gather(var, val, samps, approx) %>% 
  mutate(var = if_else(var=="samps", "True", "Approximate")) %>% 
  ggplot(aes(x=val, fill=var)) +
  geom_histogram(position="identity", alpha=.6, binwidth=.025) +
  facet_grid(p~N, scales="free_y") +
  theme_bw() +
  theme(panel.grid = element_blank()) +
  labs(title = "Initialized autocorrelated sampling")
...
dsim1 <- expand_grid(
  ...
  mutate(
    samps = map2(p, N, ~rBinEss(1000, .x, .y)),
    approx = map2(p, N, ~rbeta(1000, .x*.y, (1-.x)*.y)),
    )
...
plt_raw <- dsim1 %>% 
  ...
  theme_bw() +
  theme(panel.grid=element_blank()) +
  labs(title = "Raw autocorrelated sampling")


(plt_raw + plt_init) + plot_layout(guides="collect") & theme(legend.position = 'bottom') & labs(fill = "Samples", x="p", y="Count") & theme(axis.text.x = element_text(angle=90, vjust=.5))
```
...
The Bayesian adjustment process hypothesized by the Bayesian Sampler model makes two clear predictions about the distribution of participants' responses. First, it implies a truncated range of possible responses. Second, assuming that $\beta$ is constrained to be a relatively small value, then under the Bayesian Sampler model, $N$ influences both the degree to which responses are shrunk toward .50 and the variability of those responses trial-to-trial. Thus, participant's inferred $N$ values should be somehow correlated with the noise in their trial-level responses. In contrast, under the noise-based account of the PT+N model, there is no truncation of responses and no predicted correlation between shrinkage and response variability.

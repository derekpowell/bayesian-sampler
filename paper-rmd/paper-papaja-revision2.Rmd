...
plt_d_avgmodel + plt_d_trialmodel + plot_layout(ncol=1, guides="collect") & labs(y = "Density", x="Estimate") & theme_bw() & theme(panel.grid=element_blank())
...
t2_df %>% 
  ...
  rename(Model = model) %>%
  kbl(
    caption="Bayesian model comparison results with best scoring model in bold face. Models are compared based on $\\widehat{\text{elpd}}_{\text{LOO}}$ and the correlation between their predictions for each participants' query-level average ($r_{\\text{query}}$) as well as overall query-level averages $r_{\\text{query-avg}}$.", 
    digits=3, 
    booktabs=T,
